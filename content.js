...
let Content = {
  array_sum: {
    instructions:
      "The following JavaScript-function should calculate the sum of all integers given as an array. There seems to be an error and your task is to fix the function.",
    initialJs:
  ...
  easy_points: {
    instructions:
      "Your task is to call <code>alert</code> function with the argument 'I want points!'.",
    initialJs: "",
    preExecuteJs:
      "" +
  ...
  string_handling: {
    instructions:
      '<code>message</code> variable should have the value: <code>"This is easy", the student shouted</code>',
  ...
  double_number_function: {
    instructions:
      "Define a function called <code>doubleNumber</code>, that takes a number as parameter and returns the given number multiplied by 2",
    initialJs: "",
    postExecuteJs:
      ";\n" +
  ...
  half_number_function: {
    instructions:
      "Define a function called <code>halveNumber</code>, that takes a number as parameter and returns the given number divided by 2",
    initialJs: "",
    postExecuteJs:
      ";\n" +
      "var a = Math.floor((Math.random() * 100) + 1);\n" +
  ...
  string_printing: {
    instructions:
      'You should print the following message with console.log: <code>"This is easy", the student shouted</code>',
    initialJs: "",
    preExecuteJs:
      "" +
    ...
    description:
      "Get some easy points by calling console.log with correct argument",
  ...
  conditional_temperature: {
    instructions:
      "Write a function called <code>readTemperature</code> that takes a temperature value (number) as parameter, and returns 'Cold', when the temperature is 15 degrees celcius or below, 'Moderate' when it is over 15 degrees but less or equal to 25 degrees, and 'Hot' when the temperature is over 25 degrees",
    initialJs: "",
    postExecuteJs:
      ";\n" +
  ...
  while_loop: {
    instructions:
      'Create a while loop, that counts down from 5 to 0 (print the numbers using console.log), and after the loop it prints "Lift off!" to the console',
    preExecuteJs:
      "" +
      "let consolePrint = [];\n" +
      "const originalLog = console.log;\n" +
    ...
    title: "While loop",
    description:
      'Create a while loop, that counts down from 5 to 0 and after the loop prints "Lift off!"',
  ...
  numbers_array: {
    instructions:
      "Define <code>numbers</code> to be an array that contains positive even integers up to (and including) ten.",
    ...
    title: "Numbers array",
    description:
      "Define an array that includes even positive integers up to (and including) ten.",
  ...
  note_array: {
    instructions:
      'Define <code>notes</code> to be an array that contains 7 "note arrays" that are pairs of note pitch and it\'s length, e.g. <code>["C5","8n"]</code>. The array must first have four sixteenth (16n) notes A4, B4, C#4, A4. Then, TWICE the eighth (8n) note E5. And finally, the half (2n) note C#4.',
    initialJs: "let notes;",
    postExecuteJs:
      ";" +
      'window["sequence"] = [];\n' +
    ...
    points: function ($element, config, accessor) {
      ...
      var p = accessor.testResults(1, function (i, args, res) {
        if (args !== undefined && args.length > 1) {
          return args[0] == correct[i][0] && args[1] == correct[i][1]
            ? i > 3
              ? 2
              : 1
            : 0;
  ...
  repeat_note: {
    instructions:
      "Create a function called <code>repeatNote</code>, that takes as parameters a <code>note</code> as string and a number <code>n</code>.</br>" +
  ...
  find_the_right_element: {
    instructions: `
    We have some really amazing blogs, but not as amazing web developers... They are trying to write a function that takes a blog <code>id</code> as argument (e.g. 'blog-1' and 'blog-2'). And then, from the element with an <code>id</code> that matches the argument, the function selects <strong>all</strong> the elements that has the attribute <code>class="content"</code>. Finally, the function returns the elements obtained from the previous step.</br></br>
  ...
  remove_element: {
    ...
    points: function ($element, config, accessor) {
      let p = accessor.testResults(10, function (i, args, res) {
        ...
        let points = args.every((item, i) =>
          item.every((key) => key !== correctKeys[i])
        )
          ? 10
          : 0;
        if (points === 0) {
          let wrongKeyboards = args.filter((item, i) =>
            item.some((key) => key === correctKeys[i])
          );
          console.warn(
            "Some of the keyboards returned the wrong keys",
            wrongKeyboards
          );
        }
        return points;
  ...
  modify_element_innerText: {
    ...
    points: function ($element, config, accessor) {
      let p = accessor.testResults(10, function (i, args, res) {
        ...
        let correctKeys1 = ["C", "C#", "D", "D#", "E"];
        let correctKeys2 = ["F", "F#", "G", "G#", "A"];
        let correctValues1 = args[0][0].every(
          (item, i) => item === correctKeys1[i]
        );
        let correctValues2 = args[0][1].every(
          (item, i) => item === correctKeys2[i]
        );

        let correctInnerText1 = args[1][0].every(
          (item, i) => item === correctKeys1[i]
        );
        let correctInnerText2 = args[1][1].every(
          (item, i) => item === correctKeys2[i]
        );
  ...
  adjust_css_classes: {
    instructions:
      "Whenever a button is clicked the function onPlay(id) that is" +
      " given below is called, passing in (as argument) the <code>id</code>" +
      " attribute of the button that's clicked. For example, <code>onPlay('C4')</code>" +
      " if the C4 button is clicked. Inside the function, this argument" +
      " is stored in the parameter named <code>id</code>." +
      " Meaning that via this <code>id</code> parameter," +
      " we can find out which buttons was clicked. <strong>Your task</strong> is to" +
      " edit the function body, in order to set a CSS class <code>last-played</code>" +
      " to the last button that was clicked. <strong>Note</strong>: multiple buttons might have" +
      " been clicked, but make sure that only the last one has the" +
      " <code>last-played</code> class at any given time." +
    ...
    preExecuteHtml:
      '<div id="keyboard" class="tone-keyboard">\n' +
  ...
  events: {
    ...
    and "mouseleave". Complete the code below so that when the "mouseenter" event fires, the
    <code>innerText</code> is set, and when the "mouseleave" event fires, the <code>innerText</code> is removed.
...
for(let i = 0; i < buttonsIds.length; i++) {
  ...
  add_event_listener: {
    instructions:
      "Create a new function and add it as an event listener" +
